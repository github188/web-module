package com.ehualu.rise.pojo.violate;

import java.io.Serializable;
import java.util.Date;

/**
 * 机动车违法与驾驶证违法综合处理-针对APP终端实际业务
 * @author Administrator
 *
 */
public class ViolationComplex implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = 4411592227639090148L;

	private String xh;//序号
	
	private String tzsh;//通知书号

	private String wfbh;//违法编号
	
	private String jdsbh;//决定书编号  
	
	private String jdslb;//决定书类别  
	
	private String lxfs;//联系方式  
	
	private String hpzl;//号牌种类  
	
	private String jdcsyr;//机动车所有人  
	
	private String jszh;//驾驶证号
	
	private String hphm;//号牌号码  
	
	private String wfdd;//违法地点  
	
	private String wfdz;//违法地址  
	
	private Date wfsj;//违法时间  
	
	private String wfxw;//违法行为
	
	private String xzqh;//行政区划  
	
	private String dsr;//当事人  
	
	private String fzjg;//发证机关  
	
	private Date clsj;//处理时间  
	
	private double fkje;//罚款金额  
	
	private String cjjg;//采集机关  
	
	private double wfjfs;//违法记分数  
	
	private String fdjh;//发动机号  
	
	private String clbj;//处理标记  
	
	private String clsbdh;//车辆识别代号
	
	private Date lrsj;//录入时间
	
	private String xxly;//信息来源
	
	private double znj;//滞纳金
	
	private double yfznj;//应付滞纳金
	
	private String jkfs;//缴款方式
	
	private String cljg;//处理机关
	
	private String fxjg;//发现机关
	
	private String cfzl;//处罚种类
	
	private String jkbj;//缴款标记
	
	private String jsjg;//接收机关
	
	private Date gxsj;//更新时间
	
	private int sjly;//数据来源0=sur(机动车违法),1=vio(驾驶证违法)

	
	
	
	
	
	public ViolationComplex(String xh, String tzsh, String wfbh, String jdsbh,
			String jdslb, String lxfs, String hpzl, String jdcsyr, String hphm,
			String wfdd, String wfdz, Date wfsj, String wfxw, String xzqh,
			String dsr, String fzjg, Date clsj, double fkje, String cjjg,
			double wfjfs, String fdjh, String clbj, String clsbdh) {
		super();
		this.xh = xh;
		this.tzsh = tzsh;
		this.wfbh = wfbh;
		this.jdsbh = jdsbh;
		this.jdslb = jdslb;
		this.lxfs = lxfs;
		this.hpzl = hpzl;
		this.jdcsyr = jdcsyr;
		this.hphm = hphm;
		this.wfdd = wfdd;
		this.wfdz = wfdz;
		this.wfsj = wfsj;
		this.wfxw = wfxw;
		this.xzqh = xzqh;
		this.dsr = dsr;
		this.fzjg = fzjg;
		this.clsj = clsj;
		this.fkje = fkje;
		this.cjjg = cjjg;
		this.wfjfs = wfjfs;
		this.fdjh = fdjh;
		this.clbj = clbj;
		this.clsbdh = clsbdh;
	}

	
	
	public ViolationComplex(String wfbh, String jdsbh, String jdslb,
			String lxfs, String hpzl, String jdcsyr, String hphm, String wfdd,
			String wfdz, Date wfsj, String wfxw, String dsr, Date clsj,
			double fkje, double wfjfs, Date lrsj, String xxly, double znj,
			String jkfs, String cljg, String fxjg, String cfzl, String jkbj,
			String jsjg, Date gxsj) {
		super();
		this.wfbh = wfbh;
		this.jdsbh = jdsbh;
		this.jdslb = jdslb;
		this.lxfs = lxfs;
		this.hpzl = hpzl;
		this.jdcsyr = jdcsyr;
		this.hphm = hphm;
		this.wfdd = wfdd;
		this.wfdz = wfdz;
		this.wfsj = wfsj;
		this.wfxw = wfxw;
		this.dsr = dsr;
		this.clsj = clsj;
		this.fkje = fkje;
		this.wfjfs = wfjfs;
		this.lrsj = lrsj;
		this.xxly = xxly;
		this.znj = znj;
		this.jkfs = jkfs;
		this.cljg = cljg;
		this.fxjg = fxjg;
		this.cfzl = cfzl;
		this.jkbj = jkbj;
		this.jsjg = jsjg;
		this.gxsj = gxsj;
	}




	public ViolationComplex() {
		super();
	}



	public String getXh() {
		return xh;
	}

	public void setXh(String xh) {
		this.xh = xh;
	}

	public String getTzsh() {
		return tzsh;
	}

	public void setTzsh(String tzsh) {
		this.tzsh = tzsh;
	}

	public String getWfbh() {
		return wfbh;
	}

	public void setWfbh(String wfbh) {
		this.wfbh = wfbh;
	}

	public String getJdsbh() {
		return jdsbh;
	}

	public void setJdsbh(String jdsbh) {
		this.jdsbh = jdsbh;
	}

	public String getJdslb() {
		return jdslb;
	}

	public void setJdslb(String jdslb) {
		this.jdslb = jdslb;
	}

	public String getLxfs() {
		return lxfs;
	}

	public void setLxfs(String lxfs) {
		this.lxfs = lxfs;
	}

	public String getHpzl() {
		return hpzl;
	}

	public void setHpzl(String hpzl) {
		this.hpzl = hpzl;
	}

	public String getJdcsyr() {
		return jdcsyr;
	}

	public void setJdcsyr(String jdcsyr) {
		this.jdcsyr = jdcsyr;
	}

	public String getJszh() {
		return jszh;
	}

	public void setJszh(String jszh) {
		this.jszh = jszh;
	}

	public String getHphm() {
		return hphm;
	}

	public void setHphm(String hphm) {
		this.hphm = hphm;
	}

	public String getWfdd() {
		return wfdd;
	}

	public void setWfdd(String wfdd) {
		this.wfdd = wfdd;
	}

	public String getWfdz() {
		return wfdz;
	}

	public void setWfdz(String wfdz) {
		this.wfdz = wfdz;
	}

	public Date getWfsj() {
		return wfsj;
	}

	public void setWfsj(Date wfsj) {
		this.wfsj = wfsj;
	}

	public String getXzqh() {
		return xzqh;
	}

	public void setXzqh(String xzqh) {
		this.xzqh = xzqh;
	}

	public String getDsr() {
		return dsr;
	}

	public void setDsr(String dsr) {
		this.dsr = dsr;
	}

	public String getFzjg() {
		return fzjg;
	}

	public void setFzjg(String fzjg) {
		this.fzjg = fzjg;
	}

	public double getFkje() {
		return fkje;
	}

	public void setFkje(double fkje) {
		this.fkje = fkje;
	}

	public String getCjjg() {
		return cjjg;
	}

	public void setCjjg(String cjjg) {
		this.cjjg = cjjg;
	}

	public double getWfjfs() {
		return wfjfs;
	}

	public void setWfjfs(double wfjfs) {
		this.wfjfs = wfjfs;
	}

	public String getFdjh() {
		return fdjh;
	}

	public void setFdjh(String fdjh) {
		this.fdjh = fdjh;
	}

	public String getClbj() {
		return clbj;
	}

	public void setClbj(String clbj) {
		this.clbj = clbj;
	}

	public String getClsbdh() {
		return clsbdh;
	}

	public void setClsbdh(String clsbdh) {
		this.clsbdh = clsbdh;
	}

	public Date getLrsj() {
		return lrsj;
	}

	public void setLrsj(Date lrsj) {
		this.lrsj = lrsj;
	}

	public String getXxly() {
		return xxly;
	}

	public void setXxly(String xxly) {
		this.xxly = xxly;
	}

	public double getZnj() {
		return znj;
	}

	public void setZnj(double znj) {
		this.znj = znj;
	}

	public double getYfznj() {
		return yfznj;
	}

	public void setYfznj(double yfznj) {
		this.yfznj = yfznj;
	}

	public String getJkfs() {
		return jkfs;
	}

	public void setJkfs(String jkfs) {
		this.jkfs = jkfs;
	}

	public String getCljg() {
		return cljg;
	}

	public void setCljg(String cljg) {
		this.cljg = cljg;
	}

	public String getFxjg() {
		return fxjg;
	}

	public void setFxjg(String fxjg) {
		this.fxjg = fxjg;
	}

	public String getCfzl() {
		return cfzl;
	}

	public void setCfzl(String cfzl) {
		this.cfzl = cfzl;
	}

	public String getJkbj() {
		return jkbj;
	}

	public void setJkbj(String jkbj) {
		this.jkbj = jkbj;
	}

	public String getJsjg() {
		return jsjg;
	}

	public void setJsjg(String jsjg) {
		this.jsjg = jsjg;
	}

	public Date getGxsj() {
		return gxsj;
	}

	public void setGxsj(Date gxsj) {
		this.gxsj = gxsj;
	}

	public int getSjly() {
		return sjly;
	}

	public void setSjly(int sjly) {
		this.sjly = sjly;
	}

	public Date getClsj() {
		return clsj;
	}

	public void setClsj(Date clsj) {
		this.clsj = clsj;
	}

	public String getWfxw() {
		return wfxw;
	}

	public void setWfxw(String wfxw) {
		this.wfxw = wfxw;
	}
	
	
}
