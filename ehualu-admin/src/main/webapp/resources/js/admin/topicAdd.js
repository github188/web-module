$(document).ready( function() {
	var reqUrl = "http://"+$(ctxip).val()+":"+$(ctxport).val()+$(ctx).val()+"/admin/upload/image";
    ...
    function insertUpload(msg) {
        ...
        //插入图片到编辑器
        var nodeImg = "<img height='400' src='"+_msg+"' id='attach_"+_msg+"'/>";
        editor.pasteHTML(nodeImg);
    }
});
$(function(){
	...
	function createAttachNode(attach) {
		...
		node+="<td><a href='#' title='"+attach.id+"' class='list_op deleteAttach delete'>删除附件</a></td>";
		node+="</tr>";
		return node;
	}
